...
Answer here:Because her we represent a set of data (continents), not just color control, as the colors here belong to the continents, and also the filling is related to the continents
But the Alpha its talking about all Graphic ( geom_density )
...
plastic_waste %>% 
  ...
  ggplot(aes(x = coastal_pop_prop, y = plastic_waste_per_cap, color = continent)) + 
    ...
    theme_minimal()
``` 
Answer: geom_smooth (aids the eye in seeing patterns in the presence of over plotting also its help us to do see the curve of the data and analysis it via a different way)
the coastal cites in Asia and north America is have the most plastic waste per capita and the lowest population
